...
import type { BaselineFile, BehaviorSnapshot } from './types.js';
...
export async function writeBaseline(checks: BehaviorSnapshot[], root = process.cwd()): Promise<string> {
  const dir = path.join(root, BASELINE_DIR); const target = baselinePath(root); const temp = `${target}.tmp-${process.pid}`;
...
export async function readBaseline(root = process.cwd()): Promise<BaselineFile> {
  ...
  if (typeof parsed !== 'object' || parsed === null || (parsed as {version?: unknown}).version !== 1 || !Array.isArray((parsed as {checks?: unknown}).checks)) throw new Error('Unsupported or invalid baseline format.');
